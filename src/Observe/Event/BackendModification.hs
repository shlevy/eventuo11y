...
-- | Modify an t'Observe.Event.EventBackend'
data EventBackendModifier r
  = -- | Ignore all instrumentation using the t'Observe.Event.EventBackend'
    --
    -- All t'Observe.Event.Event's generated by this backend will have the given
    -- reference.
    Silence !r
...
-- | Modify an t'Observe.Event.EventBackend' according to the given 'EventBackendModifier's.
--
-- This is a right fold, e.g. @modifyEventBackend [ a, b ] backend@ first
-- modifies @backend@ with @b@ and then modifies the result with @a@.
modifyEventBackend :: Monad m => [EventBackendModifier r] -> EventBackend m r s -> EventBackend m r s
modifyEventBackend [] backend = backend
modifyEventBackend (hd : rest) backend' = case hd of
  Silence r -> noopEventBackend r
  SetAncestor parent ->
    backend'
      { newEventImpl = \sel -> do
...
-- | A list of 'EventBackendModifier's that does nothing.
unmodified :: [EventBackendModifier r]
unmodified = []

-- | A list of 'EventBackendModifier's that silences events.
silence :: r -> [EventBackendModifier r]
silence r = [Silence r]

-- | A list of 'EventBackendModifier's that marks every parentless event as the child
-- of a known t'Observe.Event.Event'.
setAncestor :: r -> [EventBackendModifier r]
setAncestor r = [SetAncestor r]

-- | A list of 'EventBackendModifier's that marks every causeless event as proximately caused
-- by a known t'Observe.Event.Event'.
setInitialCause :: r -> [EventBackendModifier r]
setInitialCause r = [SetInitialCause r]
